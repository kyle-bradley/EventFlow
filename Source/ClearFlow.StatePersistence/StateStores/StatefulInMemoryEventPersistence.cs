...
public class StatefulInMemoryEventPersistence : IEventPersistence, IDisposable
{
    private readonly ILogger<StatefulInMemoryEventPersistence> _logger;
    ...
    public StatefulInMemoryEventPersistence(
        ILogger<StatefulInMemoryEventPersistence> logger)
    {
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        using (await _asyncLock.WaitAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            var eventsExcludingUpdateToPersist = newCommittedDomainEvents.SkipLast(1).ToList();
            var noNewEvents = !eventsExcludingUpdateToPersist.Any();
            if (noNewEvents)
            {
                return newCommittedDomainEvents;
            }

            var expectedVersion = eventsExcludingUpdateToPersist.First().AggregateSequenceNumber - 1;
            var lastEvent = eventsExcludingUpdateToPersist.Last();

            var updateResult = _eventStore.AddOrUpdate(id.Value, s => new ImmutableEventCollection(eventsExcludingUpdateToPersist),
                (s, collection) => collection.Count == expectedVersion
                    ? collection.Add(eventsExcludingUpdateToPersist)
                    : collection);

            if (updateResult.Last != lastEvent)
            {//Fix injected event causing this check causing false positives
